...
const handleTargetSelection = useCallback(async (targetIndex) => {
    ...
    if (!targetCard || !targetCard.id) {
        ...
        // Check if the target card is a Monster type
        if (targetCard.cardType !== 'monster') {
            toast.warn('Please select a Monster card to attack.');
            console.warn('Non-monster card selected as target. Attack aborted.');
            return;
        }
